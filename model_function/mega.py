...
class MegaModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, dropout):
        ...
        self.mega = MegaModel.from_pretrained('mnaylor/mega-base-wikitext').requires_grad_(False)

        for name, param in self.mega.named_parameters():
    ...
    def forward(self, **kwargs):
        ...
        mega_feature = self.mega(inputs, attention_mask = masks)[0]
        mega_feature, _ = self.attention(mega_feature, masks)
...
class Trainer():
    ...
    def train(self, logger = None):
        ...
        self.model = MegaModel(self.config['emb_dim'], self.config['model']['mlp']['dims'], self.config['model']['mlp']['dropout'])
        ...
        for epoch in range(self.config['epoch']):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                    os.path.join(self.save_path, 'parameter_mega.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_path, 'parameter_mega.pkl')))
        ...
        return future_results, os.path.join(self.save_path, 'parameter_mega.pkl')
