...
def train_bert(config, logger):
    ...
    model = BERTEmo_ENDEFModel(config['emb_dim'], config['model']['mlp']['dims'], config['model']['mlp']['dropout'])
    if config['use_cuda']:
        model = model.cuda()
    ...
    for epoch in range(config['epoch']):
        ...
        if mark == 'save':
            ...
            continue
    model.load_state_dict(torch.load(os.path.join(save_path, 'parameter_bertemoendef.pkl')))
    ...
    return future_results, os.path.join(save_path, 'parameter_bertemoendef.pkl')