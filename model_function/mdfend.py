...
def train_bert(config, logger):
    ...
    model = MDFENDModel(config['emb_dim'], config['model']['mlp']['dims'], config['model']['mlp']['dropout'])
    if config['use_cuda']:
        model = model.cuda()
    ...
    for epoch in range(config['epoch']):
        ...
        if mark == 'save':
            torch.save(model.state_dict(),
                os.path.join(save_path, 'parameter_mdfend.pkl'))
        elif mark == 'esc':
            break
        else:
            continue
    model.load_state_dict(torch.load(os.path.join(save_path, 'parameter_mdfend.pkl')))
    ...
    return future_results, os.path.join(save_path, 'parameter_mdfend.pkl')