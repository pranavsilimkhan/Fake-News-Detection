...
def test(dataloader, model, config):
    ...
    for step_n, batch in enumerate(data_iter):
        with torch.no_grad():
            ...
            _, __, batch_pred = model(**batch_data)
...
def train_bert(config, logger):
    ...
    model = MDFEND_ENDEFModel(config['emb_dim'], config['model']['mlp']['dims'], config['model']['mlp']['dropout'])
    if config['use_cuda']:
        model = model.cuda()
    ...
    for epoch in range(config['epoch']):
        ...
        if mark == 'save':
            torch.save(model.state_dict(),
                os.path.join(save_path, 'parameter_bigruendef.pkl'))
        elif mark == 'esc':
            break
        else:
            continue
    model.load_state_dict(torch.load(os.path.join(save_path, 'parameter_bertendef.pkl')))
    ...
    return future_results, os.path.join(save_path, 'parameter_bigruendef.pkl')